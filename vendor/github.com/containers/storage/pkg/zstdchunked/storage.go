...
type ImageSourceSeekable interface {
	...
	GetBlobAt([]ImageSourceChunk) (io.ReadCloser, string, error)
}

type ChunkedZstdDiffer struct {
	stream         ImageSourceSeekable
	manifest       []byte
	layersMetadata map[string][]FileMetadata
...
func prepareOtherLayersCache(layersMetadata map[string][]FileMetadata) map[string]map[string]*FileMetadata {
	maps := make(map[string]map[string]*FileMetadata)

	for layerID, v := range layersMetadata {
		r := make(map[string]*FileMetadata)
		for i := range v {
			r[v[i].Digest] = &v[i]
		}
		maps[layerID] = r
...
func getLayersCache(store storage.Store) (map[string][]FileMetadata, map[string]string, error) {
	allLayers, err := store.Layers()
	if err != nil {
		return nil, nil, err
	}

	layersMetadata := make(map[string][]FileMetadata)
...
func findFileInOtherLayers(file FileMetadata, root string, dirfd int, layersMetadata map[string]map[string]*FileMetadata, layersTarget map[string]string, missingDirsMode os.FileMode) (*os.File, int64, error) {
...
// paths.
func findFileOnTheHost(file FileMetadata, root string, dirfd int, missingDirsMode os.FileMode) (*os.File, int64, error) {
	hostContentDigester := digest.Canonical.Digester()
	hostContentChecksum := hostContentDigester.Hash()
	...
	checksum := hostContentDigester.Digest()
...
func setFileAttrs(file *os.File, mode os.FileMode, metadata *FileMetadata, options *archive.TarOptions) error {
...
func createFileFromZstdStream(dest string, dirfd int, reader io.Reader, missingDirsMode, mode os.FileMode, metadata *FileMetadata, options *archive.TarOptions) (err error) {
...
func storeMissingFiles(stream io.Reader, contentType string, dest string, dirfd int, missingChunks []missingChunk, missingDirsMode os.FileMode, options *archive.TarOptions) error {
	mediaType, params, err := mime.ParseMediaType(contentType)
	if err != nil {
		return err
	}
	if !strings.HasPrefix(mediaType, "multipart/") {
		if len(missingChunks) != 1 {
			return errors.Errorf("invalid response, no multipart for multiple chunks requested")
		}

		for _, mf := range missingChunks[0].Files {
			if mf.Gap > 0 {
				limitReader := io.LimitReader(stream, mf.Gap)
			...
			limitReader := io.LimitReader(stream, mf.Length())
	...
	buffered := makeBufferedNetworkReader(stream, 64, 16384)
	...
	for mc := 0; ; mc++ {
		...
		for _, mf := range missingChunks[mc].Files {
			if mf.Gap > 0 {
				limitReader := io.LimitReader(p, mf.Gap)
				_, err := io.Copy(ioutil.Discard, limitReader)
				if err != nil {
					return err
				}
			...
			limitReader := io.LimitReader(p, mf.Length())
...
func retrieveMissingFiles(input *ChunkedZstdDiffer, dest string, dirfd int, missingChunks []missingChunk, missingDirsMode os.FileMode, options *archive.TarOptions) error {
	...
	for stream == nil {
		stream, contentType, err = input.stream.GetBlobAt(chunksToRequest)
		if err != nil {
	...
	if err := storeMissingFiles(stream, contentType, dest, dirfd, missingChunks, missingDirsMode, options); err != nil {
		stream.Close()
		return err
	}
...
func safeMkdir(target string, dirfd int, mode os.FileMode, metadata *FileMetadata, options *archive.TarOptions) error {
...
func safeLink(target string, dirfd int, mode os.FileMode, metadata *FileMetadata, options *archive.TarOptions) error {
...
func safeSymlink(target string, dirfd int, mode os.FileMode, metadata *FileMetadata, options *archive.TarOptions) error {
	destDir, destBase := filepath.Dir(metadata.Name), filepath.Base(metadata.Name)
	destDirFd := dirfd
	if destDir != "." {
		f, err := openFileUnderRoot(destDir, target, dirfd, unix.O_RDONLY, 0)
...
func (d *ChunkedZstdDiffer) ApplyDiff(dest string, options *archive.TarOptions) (graphdriver.DriverWithDifferOutput, error) {
	...
	var mergedEntries []FileMetadata
