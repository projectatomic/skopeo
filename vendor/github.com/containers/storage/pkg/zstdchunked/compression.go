...
func ReadZstdChunkedManifest(blobStream ImageSourceSeekable, blobSize int64, annotations map[string]string) ([]byte, error) {
	...
	if offsetMetadata := annotations[manifestInfoKey]; offsetMetadata != "" {
		...
		reader, _, err := blobStream.GetBlobAt([]ImageSourceChunk{chunk})
		if err != nil {
			return nil, err
		}
...
// Compressor is a CompressorFunc for the zstd compression algorithm.
func Compressor(r io.Writer, metadata map[string]string, level *int) (io.WriteCloser, error) {
	if level == nil {
		l := 3
		level = &l
	}
