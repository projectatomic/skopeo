...
type storageImageDestination struct {
	...
	diffOutputs map[digest.Digest]*graphdriver.DriverWithDifferOutput `json:"-"`
}
...
// GetBlobAt converts from zstdchunked.GetBlobAt to ImageSourceSeekable.GetBlobAt.
func (f *zstdFetcher) GetBlobAt(chunks []zstdchunked.ImageSourceChunk) (io.ReadCloser, string, error) {
	var newChunks []types.ImageSourceChunk
	for _, v := range chunks {
		i := types.ImageSourceChunk{
			Offset: v.Offset,
			Length: v.Length,
		}
		newChunks = append(newChunks, i)
	}
	rc, contentType, err := f.stream.GetBlobAt(f.ctx, f.blobInfo, newChunks)
	if err != nil {
		if _, ok := err.(types.BadPartialRequestError); ok {
			err = zstdchunked.ErrBadRequest{}
		}
	}
...
func (s *storageImageDestination) PutBlobPartial(ctx context.Context, stream types.ImageSourceSeekable, srcInfo types.BlobInfo, cache types.BlobInfoCache) (types.BlobInfo, error) {
	...
	// While the PutBlobPartial() interface is generic, at this point only one format is supported (zstd:chunked), so  get a
	// zstd:chunked differ and apply the diff using it.
	differ, err := zstdchunked.MakeZstdChunkedDiffer(ctx, s.imageRef.transport.store, srcInfo.Size, srcInfo.Annotations, &fetcher)
	if err != nil {
		return srcInfo, err
	}
